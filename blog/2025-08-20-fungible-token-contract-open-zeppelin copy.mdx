...
`ownPublicKey()` is a witness function, meaning each user's frontend can return any value, so it must not be used directly for caller verification. See the [smart contract security guide](/compact/smart-contract-security#ownpublickey-is-a-witness-function) for the correct authentication pattern.
