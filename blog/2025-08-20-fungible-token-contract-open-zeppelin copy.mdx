...
:::warning[ownPublicKey() in production]

`ownPublicKey()` is a witness function, meaning each user's frontend can return any value, so it must not be used directly for caller verification. For the correct authentication pattern, see the [smart contract security](/compact/smart-contract-security#ownpublickey-is-a-witness-function) guide.
