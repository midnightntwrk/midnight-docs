...
For Cursor, Gemini/Antigravity CLI, GitHub Copilot, or any client that supports remote `mcpServers` in their JSON config, add it manually:
...
Some clients label this a remote or HTTP server and ask for a transport field; the URL is the same in every case. You can also click "Copy MCP URL" from the "Use MCP" menu and paste it wherever your client expects it. Check the documentation for your client for more information.
