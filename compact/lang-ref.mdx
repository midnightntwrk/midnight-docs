...
[A comprehensive list of keywords and reserved words](all-keywords.mdx) is available.
...
Each `for` statement takes one of the following forms:

```compact
for (const i of expression) statement

for (const i of start..end) statement
```

In the second form, each of `start` and `end` must be a literal
unsigned integer or a reference to a generic size parameter, and
`end` must be greater than or equal to `start`.
Otherwise, it is a static error.

The first form is typed by typing `expression` and verifying that it
is a `Vector` type, a tuple type that
(has a vector type)[Subtyping and least upper bounds],
or a `Bytes` type.

The second form is always well-typed.

!!!!!!!! TBD

The first form is evaluated by evaluating `expression`
...
!!!!!!!! TBD this applies to to unsigned integers and should be adjusted accordingly
