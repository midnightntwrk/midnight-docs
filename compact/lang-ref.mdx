...
String literals produce bytes values; Compact has no dedicated String type.
...
The static type of a string literal is `Bytes<n>`, and its value is a bytes
value containing its UTF-8 encoding.
...
value is a bytes value containing the UTF-8 encoding of `s` followed by `0`
...
!!!!!!!! TBD Kent needs to review and revise as necessary

!!!!!!!! TBD mention that a variable reference can refer to a generic size parameter
...
type `Uint<8>` to to the new tuple, i.e., the elements of the bytes value
of `expr`.
...
New Bytes values are created with expressions of the form
`Bytes [tuple-arg, ...]`.
...
(if not, it is static error), and the result is a new Bytes value of type
...
- mention of cast of state-ledger types ?
