...
The provider packages includes functionality for interacting with the Midnight network.
...
The network ID sets the network identifier for the SDK. To perform operations on the Midnight network, you need to set the appropriate network ID. Other Midnight.js libraries and components will use the same network ID to interact with the network.
