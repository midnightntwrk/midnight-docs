...
It supports the three-token system that powers Midnight: unshielded tokens (including NIGHT), shielded tokens with zero-knowledge proofs, and DUST for transaction fees.
...
For complete package details, see the [wallet SDK release notes](../../relnotes/wallet/wallet-1-0-0).
...
The swap process involves two parties: the initiator creates a partial transaction offering tokens in exchange for different tokens, and the counterparty completes the swap by balancing the transaction with their own inputs. The wallet handles all cryptographic operations and ensures both parties receive their expected tokens atomically.
