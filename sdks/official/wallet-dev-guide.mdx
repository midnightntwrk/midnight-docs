...
The wallet SDK ships as one barrel package, `@midnightntwrk/wallet-sdk`, which re-exports every sub-package listed above. Install it at the version in the [release compatibility matrix](../../relnotes/support-matrix), together with the ledger package:

<Tabs>
  <TabItem value="npm">
    ```bash
    npm install @midnightntwrk/wallet-sdk@1.2.0 @midnight-ntwrk/ledger-v8
    ```
  </TabItem>
  <TabItem value="yarn">
    ```bash
    yarn add @midnightntwrk/wallet-sdk@1.2.0 @midnight-ntwrk/ledger-v8
    ```
  </TabItem>
</Tabs>

:::note
The matrix version (1.2.0) is the version of the barrel package. The sub-packages have their own version lines (for example, `wallet-sdk-facade` 4.x and `wallet-sdk-hd` 3.x), so do not install a sub-package at the barrel version. Install the sub-packages individually only if you need to pin one of them; take each version from that package's entry in the [wallet SDK release notes](../../relnotes/wallet/).
:::
...
The example below shows how a DApp finds an installed wallet, connects to it, and sends a NIGHT transfer. For a complete React walkthrough, see [Connect a React DApp to a wallet](/guides/react-wallet-connect).

```typescript
import { nativeToken } from '@midnight-ntwrk/ledger-v8';

// Wallets inject their Initial API under window.midnight, each keyed by a
// generated UUID rather than a fixed name, so enumerate the entries and let the
// user choose when more than one wallet is installed.
const wallets = Object.values(window.midnight ?? {});
const wallet = wallets[0];

if (!wallet) {
  console.error('Please install a Midnight wallet such as Lace');
...
try {
  ...
  // Initiate a payment. makeTransfer balances and proves the transaction
  // (the wallet shows an approval prompt) and returns the serialized result.
