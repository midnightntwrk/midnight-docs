...
The wallet SDK ships as one barrel package, `@midnightntwrk/wallet-sdk`, which re-exports every sub-package listed above. Install it together with the ledger package:

<Tabs>
  <TabItem value="npm">
    ```bash
    npm install @midnightntwrk/wallet-sdk@VERSION @midnight-ntwrk/ledger-v8
    ```
  </TabItem>
  <TabItem value="yarn">
    ```bash
    yarn add @midnightntwrk/wallet-sdk@VERSION @midnight-ntwrk/ledger-v8
    ```
  </TabItem>
</Tabs>

Replace `VERSION` with the wallet SDK version in the [release compatibility matrix](../../relnotes/support-matrix).

:::note
The matrix lists the version of the barrel package. The sub-packages have their own version lines, so do not install a sub-package at the barrel version. Install a sub-package individually only if you need to pin it, and take its version from the [wallet SDK release notes](../../relnotes/wallet/).
:::
