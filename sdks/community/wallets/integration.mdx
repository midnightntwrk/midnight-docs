...
How to connect a community wallet in a browser DApp using the DApp Connector API (CAIP-372): discovery, connect, ZK proving, fees, and portable React code. For choosing a wallet first, see the [overview](./wallets-overview.mdx); for no-extension, headless, or agent flows, see the [CLI and MCP workflow](./cli-and-mcp.mdx).
