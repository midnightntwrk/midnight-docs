...
Prefer discovering the wallet key from `window.midnight` (or the widget's own discovery helpers) rather than hardcoding `mnLace`. Friendly keys such as `mnLace` still appear in some wallets today, but UUID / `rdns` injection is the durable path — see the React wallet-connect guide and [PR #1406](https://github.com/midnightntwrk/midnight-docs/pull/1406).
...
function ConnectButton() {
  ...
  // Discover rather than hardcode: prefer Object.keys/values(window.midnight)
  // or match on name/rdns. Fall back to a known friendly key only if present.
  const walletKey =
    Object.keys(window.midnight ?? {}).find((k) => k === "mnLace" || k === "1am") ??
    Object.keys(window.midnight ?? {})[0];
  if (!walletKey) return <button disabled>No Midnight wallet</button>;
  return <button onClick={() => connectWallet(walletKey, "preprod")}>Connect</button>;
}
```
