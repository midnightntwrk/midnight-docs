...
- **Supports:**
  ...
  - Injects a standard connector under `window.midnight` (historically the friendly key `mnLace`; prefer enumerating `Object.values(window.midnight)` / matching `rdns` or `name` rather than hardcoding).
