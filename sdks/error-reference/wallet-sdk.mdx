...
**Fix**: The original transaction is no longer relevant. If an unexpected replacement occurred, investigate which process submitted a conflicting transaction.
...
**Fix**: If rollback fails repeatedly, the local UTXO state might be inconsistent. In this case, you may need a full resync.
...
The capabilities package exposes 3 error types.
...
The utilities package exposes 5 error types.
...
The runtime package exposes 1 error type.
...
The address format package exposes 5 error types.
...
<summary>All 29 `_tag` values, alphabetically sorted</summary>
