...
These errors occur when a transaction is submitted to the network but fails during execution or validation.
...
- `circuitIds`: Circuits that are undefined or have mismatched verifier keys
...
Use normal `try` / `catch` and `instanceof` (or `error.name` when catching `TxFailedError` generically):
