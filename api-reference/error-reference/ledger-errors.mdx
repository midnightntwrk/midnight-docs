...
Understanding the chain from top-level to low-level is essential when reading nested error output. A `MalformedTransaction::InvalidProof` near the top of a chain may trace back to a deep `OnchainProgramError::Decode` inside a contract transcript execution.
...
19 variants, parameterized as `TransactionInvalid<D>`. These are state-application failures that occur when the ledger applies a structurally valid transaction to the current state and rejects it due to state inconsistencies.
...
| `Zswap` | A Zswap sub-operation failed (wraps `zswap::TransactionInvalid`). | See the Zswap errors section for details. |
| `Transcript` | The ledger rejected a contract transcript execution (wraps `TranscriptRejected`). | See `TranscriptRejected` for details. |
...
17 variants, parameterized as `OnchainProgramError<D>`. These are the Impact VM on-chain execution engine errors, surfaced when contract transcript execution fails inside the VM.
...
Five variants, parameterized as `TranscriptRejected<D>`. Wraps `OnchainProgramError` and represents failures during on-chain contract execution.
...
| `MerkleTreeError` | A Merkle tree update failed during Zswap state application. | For more details, see `merkle_tree::InvalidUpdate`. |
...
10 variants covering errors in protocol-level system transactions such as epoch transitions and committee updates.
...
| `WrongTransactionType` | The caller submitted a non-system transaction where the protocol requires a system transaction, or vice versa. |
...
| Variant | Description |
|---------|-------------|
| `Success` | All segments executed successfully and the ledger applied all effects. |
| `PartialSuccess` | Guaranteed segments succeeded but one or more fallible segments failed. The ledger applies guaranteed effects and discards fallible effects. |
| `Failure` | The ledger rejected the transaction entirely (typically due to `MalformedTransaction` or guaranteed segment failure). No state changes occur. |
...
| `NoSuitableCoins` | No coins of the required type or denomination are available. |
| `ProofGenerationFailed` | The client-side proof generation step failed. |
| `InvalidRecipient` | The recipient address does not conform to the expected format or the node does not support it. |
| `SerializationError` | Failed to serialize the transaction for submission. |

#### `TransactionProvingError`

Errors during the proving phase. Note that `ProvingError` is an opaque `anyhow::Error` alias.
...
| Variant | Description |
|---------|-------------|
| `DecodeError` | The indexer could not decode an event from its on-chain representation. |
...
| `UnknownEventType` | This client version encountered an event type it does not recognize. |
...
Errors in local DUST state management on the client side.

| Variant | Description |
|---------|-------------|
| `KeyNotFound` | The DUST key is not found in local state. |
| `RegistrationExpired` | The stored DUST registration has expired. |
| `CorruptedState` | Local DUST state data has corruption and the client cannot read it. |
