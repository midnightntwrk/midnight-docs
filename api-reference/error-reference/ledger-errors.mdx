...
Errors that occur during the proving phase are listed below. 

:::note

`ProvingError` is an opaque `anyhow::Error` alias.
...
Errors that occur during DUST coin spend operations are listed below.
...
Errors that occur in local DUST state management on the client side are listed below.
