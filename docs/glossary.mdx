...
A block is a collection of validated transactions grouped together and added to the blockchain in sequence. Each block contains a reference to the previous block, forming a chain. In Midnight, blocks are produced by active nodes and finalized through the network's consensus mechanism.
...
A cryptographic primitive that allows a party to commit to a chosen value while keeping it hidden, with the ability to reveal the value later. A commitment scheme has two key properties: binding, meaning the committed value cannot be changed after the fact, and hiding, meaning the commitment reveals nothing about the value before it is opened. Midnight uses commitments extensively to represent private state on the public ledger without disclosing the underlying data.
