...
- [`persistentHash`](../../compact/standard-library/exports#persistentHash),
  a building block to hash binary data
- [`persistentCommit`](../../compact/standard-library/exports#persistentCommit),
...
[`MerkleTreePath<n, T>`](../../compact/standard-library/exports#merkletreepath) type, the
[`merkleTreePathRoot<n, T>()`](../../compact/standard-library/exports#merkle_tree_path_root)
...
the [ledger data types](../../compact/reference/ledger-adt) specification.
...
export circuit check(item: Field): [] {
  ...
  assert(path.leaf == item, "path is not for this item");
...
The `path.leaf == item` assert binds the proof to `item`. `findItem` is a witness, so without that line any valid path in the tree would satisfy `checkRoot`, whichever item it belongs to. See [Restricting a circuit to a group](../../guides/security-best-practices#restricting-a-circuit-to-a-group) in the security guide.
...
export circuit increment(): [] {
  ...
  assert(authPath.leaf == publicKey(sk), "path is not for this key");
  assert(authorizedCommitments.checkRoot(
    disclose(merkleTreePathRoot<10, Bytes<32>>(authPath))), "not authorized");
  const nul = nullifier(sk);
  assert(!authorizedNullifiers.member(disclose(nul)), "already incremented");
...
The `authPath.leaf == publicKey(sk)` assert binds the path to the caller's key. The tree's leaves are public, so anyone can compute a valid path for an authorized key, and `findAuthPath` is a witness the prover controls. Without that line, a secret key that was never authorized could pass with someone else's path, and its fresh nullifier would not stop it. See [Restricting a circuit to a group](../../guides/security-best-practices#restricting-a-circuit-to-a-group) in the security guide.
