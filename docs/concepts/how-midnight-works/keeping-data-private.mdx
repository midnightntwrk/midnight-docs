...
The `authPath.leaf == publicKey(sk)` assert binds the path to the caller's key. Merkle paths are public tree data and `findAuthPath` is a witness, so without that line anyone who observed a valid path in a public transaction could replay it with an unauthorized secret key. See [Restricting a circuit to a group](../../guides/security-best-practices#restricting-a-circuit-to-a-group) in the security guide.
