...
Midnight's model is different. Midnight [splits smart contract interactions](/concepts/how-midnight-works/smart-contracts#transcripts-and-zk-snarks) into three parts: a `local` part that runs on the user's machine, a `ledger` part that records public state, and a `circuit` part that encodes the rules connecting the two. The blockchain does not need to see every private input to validate an interaction. It needs a proof that the interaction followed the rules and a public transcript it can apply to the ledger state.
...
- **`witness`**: private data known only to the party executing the interaction, provided by callbacks on the user's machine.
...
A value meant to stay private should not become public by accident. The network should not accept a public state transition without proof that the interaction satisfied private-side rules. Developers should be able to see, from the shape of the smart contract, where those boundaries are.
...
Compact enforces explicit disclosure through the [`disclose()` keyword](/compact/reference/explicit-disclosure). The compiler rejects any program that writes witness-derived data to the ledger, returns it from an exported circuit, or passes it to another contract without a `disclose()` wrapper. Privacy is the default; disclosure requires a deliberate declaration.
...
- [Example contracts](/examples/contracts/calculator): browse Compact contracts for elections, auctions, token transfers, and more.
