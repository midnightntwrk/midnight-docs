...
| **Example platforms** | Midnight (Compact), Zcash (Halo 2), Aztec (Noir), Aleo (Leo), Mina | Starknet (Cairo), built on StarkWare's proving stack |
...
Platforms built on zk-SNARKs include Midnight, Zcash, whose current shielded protocol uses Halo 2, Aztec with its Noir language, Aleo with Leo, and Mina, which builds its entire chain on recursive proofs.

## What is a zk-STARK?

A zk-STARK is a zero-knowledge proof that requires no trusted setup. It relies on publicly verifiable randomness and hash functions rather than elliptic-curve assumptions, which is also why STARKs are considered quantum-resistant.
...
**Typical platforms.** zk-SNARKs power Midnight, Zcash, Aztec, Aleo, and Mina. zk-STARKs power Starknet. Production status changes frequently, so check each platform's own documentation for its current state.
...
Some systems use both. ZKsync proves with a STARK-based prover and wraps the result in a final SNARK so Ethereum can verify it cheaply, and RISC Zero compresses its STARK proofs into a small SNARK for on-chain verification. The combination captures STARK-style proving scalability and SNARK-style verification cost.
...
zk-SNARKs are used by Midnight, Zcash, Aztec, Aleo, and Mina, among others. zk-STARKs are pioneered by StarkWare and power Starknet, with its Cairo language. Some systems use both approaches together: ZKsync and RISC Zero prove with STARKs and wrap the result in a SNARK for cheap on-chain verification. Adoption details change, so check each platform's own documentation for its current state.
...
- [ZKsync documentation](https://docs.zksync.io) and [RISC Zero documentation](https://dev.risczero.com): systems combining STARK proving with SNARK verification.
- [Aztec documentation](https://docs.aztec.network), [Aleo documentation](https://developer.aleo.org), and [Mina documentation](https://docs.minaprotocol.com): further zk-SNARK platform references.
