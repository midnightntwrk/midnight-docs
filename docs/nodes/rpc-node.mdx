...
* `--rpc-port 9933`: The TCP port the RPC server listens on.
...
* `--rpc-external`: Binds the RPC server to all interfaces so it is reachable from external networks. Without this flag, RPC stays on `127.0.0.1`.
* `--ws-external`: Makes the WebSocket interface reachable from external networks.
