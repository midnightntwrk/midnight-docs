...
Below is a list of the Midnight node specific errors categorized by types and status code. The error tables are grouped by code range to help you quickly identify the type of error and how to fix it.
...
Most deserialization errors occur due to version mismatch. Always refer to the [compatibility matrix](../relnotes/support-matrix) to ensure you are using compatible version of Midnight blockchain components.
...
Errors that occur when the node rejects a transaction that was applied to the ledger state but violates a ledger invariant.
...
| 113 | `VerifierKeyNotPresent` | Referenced verifier key not found | Deploy the contract with theverifier key before calling the circuit |
...
Errors from the multi-collective goverance authority pallet.
