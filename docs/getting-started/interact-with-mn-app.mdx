...
Continue from deployment and build a command-line interface (CLI) to interact with a live Hello World contract on Midnight Testnet.
...
- Project with all dependencies installed via `npm install`

## Create an interactive CLI script

Add a script to project that provides an interactive menu for calling contract functions.

### Update package.json

Add CLI script to `scripts` section in `package.json`:
...
**Create a new TypeScript file for the CLI application.** This file contains all the logic for interacting with the deployed contract.
...
<Tabs>
<TabItem value="store" label="Store Message">
...
<TabItem value="read" label="Read Message">
...
You can call `storeMessage()` multiple times. Each call creates a new transaction that updates contract state with latest message.
:::

## Next steps

CLI successfully interacts with a privacy-preserving smart contract that accepts user input. Explore advanced features:
