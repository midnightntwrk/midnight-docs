...
Install the toolchain version listed for your target network in the [compatibility matrix](../relnotes/support-matrix), currently 0.31.1:

```bash
compact update 0.31.1
```

This command downloads that version of the Compact compiler and sets it as the default. A bare `compact update` installs the newest published release, which can target a ledger version that is not yet deployed on the public networks, so pin the version explicitly unless you are working against a pre-release network.
...
- [Create your first Midnight contract](./hello-world)
