...
Add basic validation to ensure message quality:

```compact
export circuit storeMessage(customMessage: Opaque<"string">): [] {
    // Validate the message isn't empty
    assert(customMessage != "", "Message cannot be empty");
    message = disclose(customMessage);
}
```

These patterns show how flexible Compact circuits can be while maintaining privacy and security.
