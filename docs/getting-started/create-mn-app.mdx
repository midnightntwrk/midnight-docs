...
An `import` statement for *Compact Standard Library* is required. This imports Compact's standard library which provides built-in types and functions needed for things like defining byte arrays.
...
To store a simple message, such as the string "Hello World," in the contract's state, first determine the correct data type. Compact provides several primitive types, including `Bytes<n>`, which represents a fixed-length byte array of size `n`.

Since the string "Hello World" contains 11 characters, use the type `Bytes<11>`. Declare a `ledger` variable named `message` to hold this value.
...
### 7. Export a circuit named `storeMessage`

Inside the circuit assign the `message` a string such as `"Hello World"`:
...
Your contract should not look similar to this:
...
Compact smart contracts need to be compiled because the Compact language is designed to express privacy-preserving logic that is enforced using zero-knowledge proofs (ZKPs). The compilation process serves several urposes:
...
- *Creation of TypeScript/JavaScript APIs*: The compiler also outputs strongly typed APIs for use in DApp frontends. This allows developers to interact with the contract from TypeScript or JavaScript code. This ensures type safety and integration between the smart contract and the application.
