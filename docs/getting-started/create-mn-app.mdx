...
Compact smart contracts can include a `pragma language_version` directive to specify the language version with which the contract is compatible. This is optional but recommended as a best practice. Use the exact language version of the compiler you are using to protect against breaking changes in future versions.
