...
The local devnet is the default. Public testnets are opt-in: pass `--network` (`undeployed`, `preview`, or `preprod`) to the project's `npm run setup` script. `--network` is a setup-script argument, not a `create-mn-app` option.
