...
The `docker-compose.yml` file defines a local devnet, a Midnight node, indexer, and [proof server](../guides/proof-server) that runs in Docker. The proof server generates Zero Knowledge (ZK) proofs for smart contracts on the Midnight network. The devnet must be running before you can deploy or interact with the contract.
