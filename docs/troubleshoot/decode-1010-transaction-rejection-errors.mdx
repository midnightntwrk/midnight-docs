...
Confirm that the proof server is running a version compatible with the node and SDK. See [run a proof server](../guides/development-environment.mdx#run-the-proof-server) and the [release compatibility matrix](../relnotes/support-matrix.mdx).
