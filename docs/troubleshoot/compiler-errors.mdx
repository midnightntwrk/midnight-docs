...
**Triggers**: The contract makes a cross-contract call while compiling for a ledger version earlier than 9, or with ZKIR v2 output. Cross-contract calls arrived with ledger version 9 and ZKIR v3 (toolchain 0.33.0 and later).

**Fix**: Check which ledger version your target network runs in the [compatibility matrix](../relnotes/support-matrix). If it is earlier than 9, restructure the contract to avoid cross-contract calls. If it is 9 or later, use a toolchain that supports it and compile with `--feature-zkir-v3`; see [Cross-contract calls](../compact/reference/compact-reference#cross-contract-calls) in the language reference and the [Compact release notes](../relnotes/compact).
