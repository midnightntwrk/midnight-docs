...
description: Common Compact compiler errors and their solutions, including lexer, parser, witness, disclosure, ZKIR, and runtime errors with fix instructions.
...
### Cross-contract calls not supported by this toolchain

**Message**: `"cross-contract calls are not yet supported"`

**Triggers**: The contract makes a cross-contract call while compiling with a toolchain earlier than 0.33.0, or with ZKIR v2 output. Toolchain 0.33.0 added cross-contract calls together with ledger version 9 and ZKIR v3.

**Fix**: If your target network runs ledger 8 (the version on every public network at the time of writing, see the [compatibility matrix](../relnotes/support-matrix)), restructure the contract to avoid cross-contract calls. If you are targeting ledger 9, update to toolchain 0.33.0 or later and compile with `--feature-zkir-v3`; see [Cross-contract calls](../compact/reference/compact-reference#cross-contract-calls) in the language reference and the [toolchain 0.34.0 release note](../relnotes/compact/toolchain-0.34.0).
