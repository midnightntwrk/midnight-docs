...
- A compiled Hello World contract in `contracts/managed/hello-world/` (complete the [build your first contract](../getting-started/hello-world) tutorial first)
- Node.js version 20.x or higher installed. Install it using [NVM](https://github.com/nvm-sh/nvm)
- Docker installed and running (required for the [proof server](./run-proof-server))
- Basic understanding of TypeScript and command-line operations

On your project's root directory, initialize an npm project with default settings. This will create a `package.json` file:

```bash
npm init -y
```

Then create the `src` directory that will hold your deployment scripts:
...
First, add all the dependencies needed for wallet management, contract deployment, and network connectivity.
...
Create a TypeScript configuration file that defines how your deployment scripts will be compiled.
...
Now, create a shared utilities file that handles wallet creation, key derivation, and provider setup. 
This code will be reused across deployment and interaction scripts for your contract.
...
- Configures WebSocket support for real-time blockchain updates.
- Defines network endpoints for the Preprod indexer, node, and proof server.
- Loads your compiled contract and prepares it for deployment.

Now add the wallet creation functions:
...
Finally, add the transaction signing and provider setup functions:
...
Now create the main deployment script that handles the entire deployment process.
...
- Prompts the user to create a new wallet or restore from an existing seed.
- Generates a cryptographically secure random seed for new wallets.
- Creates the wallet and syncs it with the Preprod network.
- Displays the wallet address and current balance.
...
- **Funding**: If the wallet has no balance, it displays instructions to visit the Preprod faucet and waits for funds to arrive.
...
Finally, add the actual deployment code:
...
Now you're ready to deploy your contract to Preprod.
...
1. **Wallet creation**: Choose to create a new wallet or restore from an existing seed. If creating a new wallet, save the displayed seed as you'll need it for later interactions with the contract.
2. **Funding**: If your wallet has no balance, the script displays your address and waits. Visit the [Preprod faucet](https://faucet.preprod.midnight.network/) to request test tokens.
...
- **seed**: Your wallet seed (stored for convenience in development—never commit this to version control in production).
...
- Wait longer for DUST generation (can take several minutes).
...
If the script reports missing contract files:
...
Now that you've deployed your contract to Preprod, you can:

- **Build a CLI**: Create a command-line interface to interact with your deployed contract and call the `storeMessage` circuit.
- **Build a DApp**: Create a web interface using [React and the DApp Connector API](../guides/react-wallet-connect).

## Reference

- [Compact language reference](../compact/)
- [Wallet SDK documentation](/sdks/official/wallet-developer-guide)
- [Midnight.js contracts API reference](/api-reference/midnight-js)
