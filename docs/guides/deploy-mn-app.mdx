...
In the code above, we are importing the necessary modules for wallet management, contract deployment, and network connectivity.
The global `WebSocket` assignment is necessary for the wallet sync to work in Node.js.

Below the imports, we configure the network endpoints.
...
Below the network configuration, we load the compiled contract and prepare it for deployment.
