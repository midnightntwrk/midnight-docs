...
4. Derive the wallet's three key roles from a seed. One seed produces the shielded (`Zswap`), unshielded (`NightExternal`), and `Dust` key sets by hierarchical deterministic derivation. Read the seed from the environment so re-runs reuse the same wallet, and generate one only on the first run:
...
6. Print the unshielded address, fund it from the [faucet](#getting-tnight-from-the-faucet), and watch for the funds. Balances stream in while the wallet is still syncing, and a balance only ever counts UTXOs from block-confirmed transactions, so a positive balance on a newly funded wallet means the tokens have arrived even though sync is still running.

   Balances arrive as `bigint` values in the smallest denomination. NIGHT divides into 10<sup>6</sup> STAR, so the faucet's 1,000 tNIGHT reads as `1000000000`. DUST divides into 10<sup>15</sup> SPECK. Convert before displaying:
