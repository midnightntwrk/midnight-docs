...
- A local [proof server](./proof-server) on port 6300, for the wallet SDK procedure.
...
2. Start your local [proof server](./proof-server) and confirm it answers on port 6300:
...
| Connection refused on port 6300 | The proof server is not running; see [Run the proof server](./proof-server). |
...
- [Run the proof server](./proof-server): the local proof server the SDK path depends on.
