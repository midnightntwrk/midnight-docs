...
Reading the ledger has two consequences. A change reaches your code only after several blocks of confirmation, so a chain reorganization cannot leave your database holding state the chain later drops. And your code sees exactly what the contract discloses, which makes [Ledger schemas and contract design](#ledger-schemas-and-contract-design) a contract decision.
...
{/* _mod-docs-content-type: CONCEPT */}
## Ledger schemas and contract design

The `ledger` option is the interface between a Compact contract and everything that reads it. For a counter with one numeric field the choice makes no difference. For a real contract it shapes how you write the contract itself.

EffectStream matches schema keys to ledger fields by position. The first key reads the first field, the second key reads the second, and so on until your schema runs out. This positional match replaces a compiler-generated decoder, which is why a node needs no contract artifacts.

A schema accepts these types:
...
A `struct`, `Vector`, or `enum` fails at parse time. Two things follow, and both are decisions you make in Compact rather than in TypeScript.

Fields at the end cost nothing to leave out, since the reader stops where your schema does. A field the schema cannot express blocks every field after it, because there is no way to skip a position. A struct sitting third in your ledger therefore costs you the fourth field and the fifth as well. Put the fields an indexer reads at the top of your ledger, and structs and digests at the bottom.

When a struct does need indexing, flatten it into sibling top-level declarations keyed the same way. A `Map<Bytes<32>, GameState>` holding four small numbers becomes four `Map<Bytes<32>, Uint<8>>` declarations, which costs nothing on-chain:
...
For a contract you cannot reshape, EffectStream's full configuration API uses the compiler-generated decoder instead, which reads anything Compact can express.
...
| Parse error naming an array where a scalar belongs | A `struct` or `Vector` sits at a schema position | Reorder the contract's ledger, or flatten the struct; see [Ledger schemas and contract design](#ledger-schemas-and-contract-design) |
