...
To get started, clone the [Midnight local network repository](https://github.com/midnightntwrk/midnight-local-network):

```bash
git clone https://github.com/midnightntwrk/midnight-local-network.git # TODO: repo not moved to midnight org yet
...
1. Fund from a config file
2. Fund by public key
...
You will be prompted to enter the path to the config file.
...
The tool will then fund the wallets from the config file.
...
You will be prompted to enter the Bech32 addresses of the wallets you want to fund, separated by commas.
...
:::tip
If you're using a custom wallet, see our guide to learn [how to programatically register for DUST generation](./generating-dust-programatically).
:::
...
If you only need the Docker containers running without the interactive menu for wallet initialization or funding, you can use Docker Compose directly:
...
```
Operation failed: Expected undeployed address, got preprod one
```

It means you're trying to fund a wallet with a preprod address. You need to use the local network address instead.

If you're using the Lace wallet, you need to configure the wallet to use the local network. 
...
After that, you'll see your wallet addresses for the local Undeployed network. Make sure to use the Unshielded wallet address for the local network.
...
Now that you have a local network running, you can start building your DApp. See our guide to learn [how to deploy a contract to the Midnight blockchain](./deploy-mn-app).
