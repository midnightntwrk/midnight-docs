...
3. Point the configuration at Preprod: `setNetworkId('preprod')`, the Preprod indexer and node endpoints from the [environment reference](./networks-and-environments#environment-reference), and a proof server you run yourself, following [Run a proof server](./proof-server). The proof server sees witness data in the clear, so it stays local even when the network is public.
