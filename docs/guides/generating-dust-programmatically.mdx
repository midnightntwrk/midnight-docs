...
Start by creating an empty directory for the project. All the files we'll create in this tutorial will live inside it.
...
Every Node.js project needs a `package.json` file to declare its dependencies and scripts. Create the file, then open it and paste in the contents below.
...
The proof server runs in Docker. We'll define how to run it in a small Compose file so we can start it later with a single command.
...
All the script logic lives in `src/index.ts`. Create the directory and file now — we'll fill it in section by section in the rest of this step.
...
Next we define the network endpoints in one place.
...
Midnight token balances are stored as `bigint` values in their atomic units. To display them in a human-readable form, we need two small helpers, one for NIGHT and one for DUST, that convert the raw integer into a decimal string.
...
Several of the operations in this script — building the wallet, syncing with the network, and registering NIGHT take a few seconds to complete. To give the user visual feedback, we wrap each long-running operation in a small helper that animates a spinner while it runs.
...
The script needs to ask the user a few questions interactively (whether to create or restore a wallet, what dust address to designate). Here's a small `prompt` helper that wraps Node's built-in `readline` module in a Promise so we can `await` user input.
...
When the user designates a dust address for DUST generation, we want to make sure they pasted a valid one, not a shielded or unshielded address by mistake. This helper validates the bech32m format and loops until the user provides something valid (or presses Enter to use their own dust address).
...
Once the wallet is built, it needs to catch up with the network before we can use it. The wallet emits state updates as an RxJS observable, so we wait for the first state where `isSynced` is true.
...
After the user requests tNight from the faucet, we need to wait for it to arrive. Same pattern as `waitForSync`, but this time we filter for the unshielded NIGHT balance becoming non-zero.
...
DUST accrues continuously over time, so we'll want to check the current balance on demand. This small helper grabs the latest synced state and returns the dust balance at the current moment.
...
Finally, the `main` function ties everything together. It walks through the full flow: get a seed, derive keys, build and sync the wallet, display the addresses, wait for funding if needed, prompt for a dust address, register NIGHT for DUST generation, and then enter a loop where the user can re-check their DUST balance or quit.
...
With the script in place, we now need to start the proof server so it's ready to handle the zero-knowledge proofs the script will request. Make sure Docker Desktop is open and running on your machine. Then open a second terminal window, navigate to your project folder, and start the proof server:
...
If you designated an external dust address, the script will confirm the registration and exit because DUST is a shielded token, only the wallet holding that dust secret key can see the balance.
...
**Proof server won't start or hangs:** Make sure Docker Desktop is open and running first.
