...
- The proof server running. For setup instructions, see the [proof server](./run-proof-server) guide.
...
This function is the main entry point for the CLI application. It loads the deployment information, creates a wallet, and connects to the deployed contract. 
It then enters the main menu loop, which presents options to the user and handles their choices.
