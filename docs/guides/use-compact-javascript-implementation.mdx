...
This guide shows how to use the Compact JavaScript implementation in your development workflow. You will learn how to import the implementation, create a contract instance, and call its functions from a regular JavaScript or TypeScript environment.
...
You can load the implementation like any other ES module:

```typescript
import { Contract, State, pureCircuits, ledger } from './managed/bboard/contract/index.js';
```

If you are using TypeScript, the accompanying declaration file `index.d.ts` automatically provides type hints for your contract and its methods.
...
For the bulletin board contract, create `contract/src/witnesses.ts`:
...
This makes it easy to test your contract logic off-chain with full control over inputs and without requiring a Midnight node or proof server.
...
Zero-knowledge circuits are powerful, but they are also complex and opaque. You cannot easily debug or test them directly.
...
Now you understand how to use the Compact JavaScript implementation. Explore the [bulletin board DApp](../examples/dapps/bboard) for a complete example of using the Compact JavaScript implementation.