...
This guide shows you how to run the proof server locally in order
to process transactions on the Midnight Network. The wallet communicates with the proof
...
| Docker | One command, identical behaviour across macOS, Linux, and WSL. Version-pinnable, so it stays in step with the node and indexer. Isolated, so it does not touch your system. Disposable, so a broken state is fixed by restarting a container. |
...
5. Wait for finalisation through the `publicDataProvider`.

Step 2 is the only step the proof server takes part in. If your DApp hangs, look there first.

:::note

The prover verifies its own proof before returning it. If your keys and circuit do not match, the error says check that your keys match rather than failing silently at submission. In practice this almost always means stale build artifacts. Recompile the smart contract.
...
| Preprod | `preprod` | Live pre-production network for final testing before Mainnet | You are validating close-to-production behaviour. |
...
- [Install the Midnight toolchain](/getting-started/installation)
- [Set up a local network with the quickstart](/getting-started/quickstart)
- [Networks and environments](/guides/networks-and-environments)
- [Acquire tNIGHT and tDUST](/guides/acquire-tokens)
- [Understand the DUST architecture](/concepts/dust-architecture)
