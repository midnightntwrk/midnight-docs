...
This guide explains how to create a Next.js application that connects to a Midnight wallet (for example Lace) using the DApp Connector API. 
...
- A Midnight wallet extension installed in your browser (for example [Lace](https://chromewebstore.google.com/detail/lace/gafhhkghbfjjkeiendhlofajokpaflmk))
...
Create `app/lib/selectWallet.ts` first so the button stays wallet-agnostic. Wallets inject under `window.midnight` keyed by UUID; Lace may also expose a convenience alias at `window.midnight.mnLace`, but relying on that key alone misses other wallets and can resolve to `undefined` when only the UUID key is present. Enumerate with `Object.values(window.midnight)` - the same pattern as the [React wallet-connect guide](/guides/react-wallet-connect).
...
Then create `app/components/ConnectWalletButton.tsx`:
...
export default function ConnectWalletButton() {
  ...
  const handleConnect = async () => {
    try {
      // Enumerate window.midnight - do not hardcode mnLace
      const wallet = selectWallet();

      // Connect to the specified network (use 'undeployed' for local development)
      const connectedApi = await wallet.connect("preprod");

      // Prefer unshielded address for display; request shielded only when needed
...
3. **Connection logic**: The `handleConnect` function selects a wallet by enumerating `window.midnight`, connects to the specified network, and retrieves the wallet's unshielded address.
...
- Confirm that you're using the correct network ID. 
For local development, use `'undeployed'`. For Preprod environment, use `'preprod'`.
