...
- [Docker](https://www.docker.com/) (or Docker Desktop) running, used by the orchestrator to start containerized services including the Midnight node, proof server, indexer, and database.
...
- The Compact compiler, which the orchestrator also checks on `PATH`. The template pins `0.31.1`, so run `compact update --no-set-default 0.31.1`.
...
One consequence shapes the code you write. EffectStream reads Midnight's public ledger, so values disclosed in circuits and committed to the ledger are what your state machine can see. Designing the circuit designs the sync surface.
