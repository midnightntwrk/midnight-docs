...
- [Git](https://git-scm.com/) for cloning the template repository.
- [Docker](https://www.docker.com/), only on platforms without native Midnight binaries. On Linux x86-64 and Apple silicon the orchestrator runs the Midnight node, indexer, and proof server as native binaries; on other platforms it falls back to Docker images for the indexer and proof server.
- [Bun](https://bun.sh). See [Set up Bun for Midnight development](/how-to/bun-runtime-midnight).
...
- The Compact toolchain, which the orchestrator checks on `PATH` before starting. The template's build script selects toolchain `0.33.0-rc.2` explicitly, and `compact update` cannot install prereleases, so download `compactc_v0.33.0-rc.2_<target>.zip` from the [Compact releases](https://github.com/LFDT-Minokawa/compact/releases/tag/compactc-v0.33.0-rc.2) and unzip it into `~/.compact/versions/0.33.0-rc.2/<target>/`, using the same `<target>` as the zip name, for example `aarch64-darwin`. Your default toolchain stays as it is. Confirm with `compact compile +0.33.0-rc.2 --version`, which prints `0.33.0`.
...
- [Security and best practices](./security-best-practices#on-chain-visibility): how `disclose()` permits private data to cross into public ledger state.
