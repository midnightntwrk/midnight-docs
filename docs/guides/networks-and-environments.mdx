...
- Docker installed and running, for the local network and the [proof server](./proof-server).
...
- [Run the proof server](./proof-server): the local proof server every network setup depends on.
