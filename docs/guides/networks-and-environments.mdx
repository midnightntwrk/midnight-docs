...
Older articles and tools sometimes reference a network named `testnet-02`. Midnight retired that network, and its endpoints, such as `rpc.testnet-02.midnight.network` and `indexer.testnet-02.midnight.network`, no longer resolve. Use `preview` or `preprod` instead, with the endpoints listed in the [Environment reference](#environment-reference).
...
4. Pass the endpoints to your providers. The full providers object, including private state, ZK configuration, and wallet providers, is covered in [Configuring providers for a contract](./deploy-and-operate#configuring-providers-for-a-contract):
...
**On `preview` and `preprod`**, request free tNIGHT from the network's faucet (the Preprod faucet sends 1,000 tNIGHT per request; both faucets are rate limited), then register it for tDUST generation in your wallet. The [Funding a wallet](./acquire-tokens) guide walks through the faucet, the Lace **Generate tDUST** flow, and the scriptable wallet SDK path. Test tokens carry no real value.
...
3. Point the configuration at mainnet. Set the network ID and swap the endpoints; nothing else in the DApp changes:
   ...
   The `create-mn-app` scaffold does not offer a mainnet target, so wire the providers yourself as in [Configuring providers for a contract](./deploy-and-operate#configuring-providers-for-a-contract).
...
- [ ] **The updatability decision is made.** Decide whether and how the contract can be upgraded before it holds real value. See [Contract updatability and the maintenance authority](./deploy-and-operate#contract-updatability-and-the-maintenance-authority).
...
- [ ] **Deployment authorisation is requested.** Mainnet contract deployments go through the Midnight Foundation's review: score the DApp against the [contract deployment rubric](https://github.com/midnightntwrk/midnight-improvement-proposals/blob/main/deployments/contract-deployment-rubric.md), open a pull request that adds your request under [`deployments/`](https://github.com/midnightntwrk/midnight-improvement-proposals/tree/main/deployments) in the MIP repository, and wait for deployment credentials. The review is not an audit. See [How to work together to get your app deployed](/blog/how-to-apply-for-deployment-auth).
...
- [Deploying and operating a contract](./deploy-and-operate): the complete providers object a DApp passes to the SDK, and what comes after deployment.
- [Funding a wallet](./acquire-tokens): the faucet, the Lace registration flow, and the wallet SDK path.
