...
<StepsProvider>
  <Step>Retrieve current DUST parameters (generation, decay, caps) provided by the indexer.</Step>
...
<StepsProvider>
  <Step>Use the Ledger API to estimate the DUST required for the prepared transaction. Call <code>fees(params)</code> or <code>feesWithMargin(params, margin)</code> to get the cost in SPECKs, along with a short validity window for the estimate.</Step>
