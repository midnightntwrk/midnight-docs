...
## Issuance and interaction

### Unshielded (ledger) tokens
- **What they are:** Native ledger tokens (e.g., NIGHT) tracked directly in UTxOs on the ledger.
- **Unlock/Burn:** Per token policy rules; results appear transparently in UTxOs.
- **Transfers:** Standard UTxO spends.

**Example (unshielded, no code):**
1. An unlocking transaction includes the policy script and unlocks `X` units of a token.  
2. The unlocked amount appears in the transaction output’s multi-asset bundle.  
...
- **On the ledger (unshielded):**
  - Transaction hashes, addresses, amounts, **policy IDs/asset names**, and unlock/burn events are public.
...
- **Unshielded unlock + balance:**
  - Unlock under policy P → confirm output shows +X units → indexer shows new balance under P.”
...
- **Policy ID:** Identifier governing unlock/burn rules for an unshielded token.  
