...
- **Native tokens:** `NIGHT`, `DUST`  
  - `NIGHT` is transferable.  
  - `DUST` is **non-transferable** and is **consumed** to execute transactions (see [Building DApps with DUST](/develop/tokenomics/dapps-with-dust) and [Generation and Decay](/develop/tokenomics/generation-and-decay)).

- **Contract tokens (shielded):**
  Issued and managed by smart contracts; balances/transfers are shielded (private state via views/proofs). See [Developer considerations](/develop/tokenomics/developer-considerations).

- **Custom tokens (unshielded):**  
  User-created tokens on the Midnight ledger with public UTxO semantics (visible balances/flows). *(Not Cardano Native Assets.)*

- **Custom contract tokens (shielded):**  
  User-unlocked tokens managed by contracts with private state. See **Shielded token API** (link when available) and [Developer considerations](/develop/tokenomics/developer-considerations).

- **Native unshielded tokens**:
  Ledger-level assets issued directly by the protocol, visible on-chain and transferable.
  Used for protocol operations or integrations that do not require shielding.

## Comparison (at a glance)

| Class                               | Transferable | Privacy model         | Unlock/Burn authority                  | How fees are paid | Typical use cases                              |
|-------------------------------------|--------------|-----------------------|--------------------------------------|-------------------|-----------------------------------------------|
...
- **Unshielded tokens (NIGHT, DUST, custom unshielded):**  
...
- **Policy ID:** The identifier that governs unlock/burn rules for an unshielded (ledger) token.  
