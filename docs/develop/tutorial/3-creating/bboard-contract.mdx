---
sidebar_label: "3.2 Bulletin board contract"
---

# Bulletin board contract

Just as with the counter DApp, there are separate subdirectories in
the `bboard-tutorial` directory for the contract and the user
interaction code.
...
must type or paste the Compact code into a file `bboard.compact` in
the `contract/src` subdirectory of `bboard-tutorial`. Midnight
...
To create a Midnight contract for the bulletin board scenario, you need
to identify:
...
becomes occupied), because that is the point at which the current
instance no longer corresponds to the just-removed post.

The fourth value should be a non-reversible hash of the poster's
identity and the posting instance number (the counter).  No one can
...
The contract need not initialize explicitly the `poster` field when
...
into the ledger's `poster` field to identify the user posting the
...
`public_key`, which is defined in the same contract.  Here is its
definition:
...
1. The `persistent_hash` function is defined in Compact's standard
   library.
2. Typically, you would not export helper circuits like this one,
   ...
   this case, `public_key` is exported so that its value can be logged
   by the DApp, for debugging purposes.
3. While this circuit is named `public_key`, its return value is *not*
...
to `public_key`?  The function `local_secret_key` cannot be another
...
result of `local_secret_key()` is presumed to be private.  Because this value is
passed to `public_key`, the result of `public_key` is also presumed to be
...
leak the poster's identity.  Wrapping the public key value in `disclose` tells
the Compact compiler that this disclosure is intended.

With all these tools at hand, you can write the `take_down` circuit,
...
When a DApp submits a `take_down` transaction to the Midnight network, it does
...
`public_key` to be explicitly `disclose`d in this circuit.  The `take_down`
...
After saving the preceding contract as `bboard.compact` in
`bboard-tutorial/contract/src`, compile it with the Compact compiler
...
public circuits (`post`, `take_down`, and `public_key`) when you run
