...
Learn how to resolve 403 Forbidden errors when installing Midnight packages.

## Understanding the error

When you try to install Midnight packages, you might see:
...
The most common cause is npm pointing to the wrong registry.
...
If it shows anything other than `https://registry.npmjs.org/`, that's your problem.
...
Corporate VPNs and proxies often block npm access.
...
If you're on a corporate network, disabling VPN might not be possible. In that case, proceed directly to proxy configuration.
:::

Disconnect from your VPN and test:

```bash
npm install @midnight-ntwrk/compact-runtime
```

If this works, your VPN blocks npm access. Proceed to configure proxy settings.
...
If you have conflicting configurations, reset everything.
...
:::warning Important
Back up your npm configuration before clearing it, as this will remove all custom settings.
:::
...
The following commands will delete your `node_modules` and `package-lock.json` files. Make sure you're in the correct project directory.
:::
...
# Use WSL native paths (not /mnt/c/...)
...
:::note macOS Security
You may be prompted for your system password when running this command.
:::
