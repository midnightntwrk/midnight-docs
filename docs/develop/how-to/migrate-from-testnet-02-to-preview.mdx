...
- Your wallet now has 3 addresses, Shielded, Unshielded and DUST.

### SDK breaking changes
1. **LevelPrivateStateProvider**: Must provide `walletProvider` or `privateStoragePasswordProvider`
2. **Async transactions**: `submitTx` now returns `Promise<TransactionId>`
3. **WalletProvider.balanceTx**: Returns discriminated union (BalancedProvingRecipe)
4. **Contract calls**: All contract calls now return` Promise<R>`
5. **ZswapOffer**: Empty offers return `undefined`
6. **networkId**:  Changed from enum to string literal
...
Before migrating to the preview environment, here's the minimum software version you need installed on your machine.
...
See the [types reference](https://github.com/midnightntwrk/midnight-js/blob/main/packages/types/src/providers.ts) for complete details.
...
You may need to wait 2–3 minutes for tokens to arrive.
...
**Solution**: When you see a dialog stating the binary "cannot be opened", click **Done** (do not move it to trash). 

![Click done on MacOS](../../../static/img/migration/click-done.png)

Then, open System Settings and navigate to **Privacy & Security**. Scroll to the bottom where you'll see: "compactc" was blocked to protect your Mac. Click **Open Anyway** to proceed.

![Open anyway](../../../static/img/migration/open-anyway.png)

Repeat the same process for the `zkir` binary (invoked automatically during key generation).
