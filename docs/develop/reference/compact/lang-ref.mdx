...
export circuit doesnt_work(s: Even): Odd {
...
It may optionally be annotated with a type, in which case the type is
...
- either `read_nested_counter1` or `read_nested_counter2` can be used to read the value of an existing `Counter`
...
4. When the last lookup is a read of a base type one can omit the
   ...
   `read_nested_counter1` and `read_nested_counter2` above, which have the
   same behavior.
5. For convenience, local variables can hold default ledger state type values,
   so the following definition of `init_nested_map` is equivalent to the one
   above.
   ```compact
   export circuit init_nested_map(b: Boolean): [] { 
