...
compiler looks for its declaration within the current scope.  It is a static error
if an identifier is referenced in a scope but its definition is not accessible within
that scope.  It is a static error if an identifier is bound more than once in a
...
A __tuple pattern__ destructures a tuple or a vector value and
...
A __structure pattern__ desctructures a structure value and
...
It can bring into scope a __single pattern__ with `const p = e` where `p` is the new pattern and `e` is an expression.
