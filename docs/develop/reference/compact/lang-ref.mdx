...
A Compact program is __well-typed__ if it does not contain any type error.  If
a program element such as a type declaration, expression, or statement contains
a type error, it means that the Compact compiler will not compile that program fully.

## Scope and binding

In Compact, scope defines the visibility and lifetime of an identifier.  An identifier
is a name that can refer to a variable, a type, a module, a parameter,
a generic parameter, a circuit, a witness, or a ledger.

Binding is the process of associating an identifier with a program element.
The scope of a binding is the region of code where the identifier
is "bound" to its value and can be referenced.

When a program attempts to reference an identifier, the Compact
compiler looks for its declaration within the current scope.  It is a static type error
if an identifier is referenced in a scope but its definition is not accessible within
that scope.  It is a static type error if an identifier is bound more than once in a
same scope.  However, the Compact compiler accepts multiple binding to a same
identifier for unexported circuits.

Scopes can be nested, just like nested boxes.  The outermost box is the __global
scope__ of a program.  Identifiers declared here, such as types, modules,
top-level circuits, or ledger fields, are accessible throughout the entire contract.
...
are only visible within its declaration.
- [modules](#modules-exports-and-imports): All identifiers defined inside a module are only
visible within it unless they are explicitly exported.
...
- [blocks](#statements): All identifiers defined by a `const` binding inside a block are only
visible within it.
- [`for` loops](#for-loop): All identifiers introduced in a `for` loop are only visible
inside that `for` loop.
- [anonymous circuits](#circuit-and-witness-calls): Parameters of an anonymous circuit are
only visible in its body.
- [constructor](#contract-constructor): The constructor's parameters and any `const` bindings
defined inside its body are local to the constructor.

A key principle of scoping is __shadowing__.  Shadowing occurs when an identifier in an
...
Parameters can be named or they can be patterns.  A named parameter is simply just an identifier.
A pattern is a destruction of a tuple or a structure value.
...
A __tuple pattern__ has the form `[id, ...]` where `id, ...` is a sequence of zero or more comma-separated identifiers.
...
is not a tuple type.  It is a static type error if the length of the sequence of identifiers (the
number of elements in the sequence of identifiers) is larger than the
length of the tuple type.  Both the sequence of identifiers and tuple type can have a trailing comma.
The sequence of identifiers can leave out some identifiers by dropping their identifiers to skip binding
some indices of the tuple, for example, `[x, , , y] : [Boolean, Boolean, Boolean, Boolean]` only
binds the first and last element of a tuple of four boolean values.  However, doing so in the tuple
type is a parse error.  The dropped identifiers count toward the length of the tuple pattern if there
exists a named identifier after the dropped ones.  For example, `[x, , , y]` has length 4 but
`[x, y, , ]` has length 2.
...
does not have a tuple or a structure type, respectively.  For example, `[x, [y, z]] : [Field, Boolean]`
results in a static type error since the pattern `[y, z]` must have a tuple type.

A __structure pattern__ has the form `{f, ...}` where `f, ...` is a sequence of zero or more comma-separated fields.
...
sequence of zero or more comma-separated field `f` bound to pattern `p`.  If the pattern `p` is an identifier
the field `f` is reassigned to the identifier.  It is a static type error to bind the same identifier
...
It is a static type error to reference `f` after reassigning it to another pattern,
for example, using the same structure `S`,
`export circuit bad-eg({a: b, } : S) : Field { return a; }` causes an `unbound identifier a` static type
error.  If the pattern `p` is a tuple or structure desctruction it is a static type error if the
...
circuits.](#pure-and-impure-circuits) __Non-generic__ circuit definitions have the form
...
`R` is the __return type__ of the circuit:
...
Circuits can be generic. __Generic__ circuit definitions have the form
...
A `const` binding brings a new identifier into scope and assigns an expression to it.

It can bring into scope a __single pattern__ with `const p = e` where `p` is the new pattern and it is bound to expression `e`.
...
`e` must have a tuple type, otherwise it is a static type error.  It is a static type error
...
identifiers are bound from left-to-right to elements in `[v, ...]` until there is no more identifier to bind to the next
element of `[v, ...]`.  The sequence of identifiers does not have to bind every element of a tuple, that is, it can drop
some of the identifiers to skip binding them, for example, `const [a, , b] = [1, 2, 3]` binds `1` to `a` and `3` to `b`.
These skipped identifiers are counted towards the length of the sequence of identifiers. For example, `[a, , b]` has
length `3`.
...
A `const` binding can bring into scope __multiple patterns__ with `const p = e, ...` where `p = e` is a
...
A `const` binding can optionally declare the type for each binding with `const x: T = e`. It is a static type error if `e`
does not have type `T`.  In such an error, `x` has the __actual type__ `U` where `U` is the static type of `e` but it
has the __declared type__ `T`.  In multiple identifiers `const` statments,
it is not a static type error to mix and match bindings with and without a type declaration, that is,
a binding such as `const x: Field = 2, y = true` is valid.  In a tuple pattern `const` binding, it is a static type error to
skip the type of a skipped element, for example, `const [a, , b] : [Field, , Field] = [1, 2, 3]` results in a parse error and
`const [a, , b] : [Field, Field] = [1, 2, 3]` results in a static type error.  It is a static type error to skip the
expression being bound to a skipped identifier, for example, `const [a, , b] = [1, , 3]` results in a parse error.

:::note
When multiple static type errors exist in a program it is up to the compiler implementation to decide whether to throw an error
complaining about all of them at once or pick an order for the errors. For example, in
`const [a, , b] : [Field, Field] = [1, 2]` the compiler can choose to throw the static type error that the
type annotation `[Field, Field]` must have another element or to throw the static type error that the expression `[1, 2]`
must have another element or to throw both at the same time.
:::

The name of a constant may not be reused within a block, and constants cannot be reassigned, although they can be shadowed in
a nested block.  Constant initializer expressions are evaluated when the binding statement is executed.
