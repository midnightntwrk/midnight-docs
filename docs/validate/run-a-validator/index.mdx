...
There is no slashing if one is offline; there is only the opportunity cost of not receiving additional blocks.
...
To become a Midnight validator, an entity must first fulfill the prerequisite of operating as a Cardano Stake Pool Operator (SPO). This foundational requirement ensures a baseline understanding of blockchain operations and infrastructure management.

Upon satisfying the SPO requirement, an operator can register as a candidate with the Midnight Validator Committee. This committee is implemented as a smart contract on the Cardano blockchain and is responsible for managing the rotation and selection of SPOs for block production in each epoch. Registration is facilitated through straightforward command-line interface (CLI) commands provided by the Midnight node software.

Following successful registration, the final operational step involves configuring and running the Midnight node in validator mode. This mode necessitates a persistent connection between the Midnight node and a PostgreSQL instance running cardano-db-sync. This connection is crucial for the Midnight node to accurately track and synchronize with the Cardano blockchain.
...
sequenceDiagram
    ...
    participant MidnightNode_BP as "Midnight Node (Validator)"
...
Operating a Midnight validator alongside a standard Cardano Stake Pool necessitates the continuous operation of several key services, complemented by one intermittently used component.
...
- **Midnight Node** (validator Mode): This node maintains a persistent connection to PostgreSQL to follow the Cardano chain, actively producing blocks for the Midnight network.
...
| Component | Resource | Midnight Preview / Preprod | Midnight Mainnet | Notes |
| :--- | :--- | :--- | :--- | :--- |
| **Midnight Node** | **CPU** | 4 vCPU | 8 vCPU | Validator/Block Producer node. |
| | **RAM** | 16 GB | 32 GB | |
| | **Storage** | 40 GB+ (NVMe) | 100 GB+ (NVMe) | "Free space" req. varies by chain history. |
| **Cardano Node** | **CPU** | 2 vCPU | 4 vCPU | Must match env (e.g., Preview or Mainnet). |
| | **RAM** | 4 - 8 GB | 16 - 24 GB | Mainnet RAM usage grows over time. |
| | **Storage** | 20 - 40 GB | 150 GB+ (NVMe) | Storage for the Cardano chain history. |
| **Cardano DB Sync**| **CPU** | 4 vCPU | 4 vCPU | Vital for the Midnight Node to "follow" Cardano. |
| *(PostgreSQL)* | **RAM** | 16 GB (Min) | 32 GB (Rec) | Resource intensive; RAM prevents sync lag. |
| | **Storage** | 40 GB+ | 320 GB+ (NVMe) | Stores indexed Cardano history. |
...
## Ogmios Endpoint Configuration

A reachable **Ogmios endpoint** is required to submit registration transactions using the Midnight-node `partner-chain-cli`. While we encourage operators to run a local Ogmios instance for maximum sovereignty and performance, a public endpoint is provided for convenience.

### Connection Details
* **Protocol:** HTTPS / WSS (Secure)
* **Port:** `443`
* **Public URL:** `ogmios.preview.midnight.network`

### Integration Note
When configuring your CLI or node environment, ensure your firewall allows outbound traffic on **Port 443**. If you are using the public endpoint, your connection string will typically look like this:

`https://ogmios.preview.midnight.network:443`
