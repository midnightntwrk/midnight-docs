...
**Value lives in immutable coins.** A shielded coin is described by a `ShieldedCoinInfo`: a nonce, a color, and a value. You cannot change a coin's value in place. To move part of it, you spend the coin and the standard library hands you back a new *change* coin for the remainder. A nonce keeps every coin unique, and you derive nonces deterministically with `evolveNonce` from a private seed.
...
| Committed (already in the ledger) | `QualifiedShieldedCoinInfo` (adds `mtIndex`) | `sendShielded` |

A freshly minted coin has no Merkle tree position yet, so it cannot be spent with `sendShielded`. Once it is committed on-chain it gains an `mtIndex` and becomes a `QualifiedShieldedCoinInfo`, which is what `sendShielded` requires. This tutorial mints and spends a coin in the *same* transaction, which keeps you on the fresh-coin path and avoids tracking Merkle positions across transactions. The committed path is covered in [Going further](#going-further-with-committed-transfers-and-burns).
...
The new line is the witnesses. A witness is a function that runs off-chain and feeds a value into the circuit. That value is often private data the circuit uses without publishing it, but it doesn't have to be — a witness is really just a hook for off-chain computation, and what it returns can be public or private. Privacy isn't tied to witnesses either: a value passed as an ordinary circuit parameter also stays private unless the circuit explicitly discloses it.
...
`token_color` and `initialized` play the same role as in the unshielded tutorial. Note what is *not* here: there is no balance, and no map of who-holds-what. Recording shielded balances on-chain would publish the very amounts shielding is meant to hide, so the contract keeps none. `mints` is a simple counter, used only to show on-chain state changing.
...
- `sendImmediateShielded(coin, ..., coin.value)` spends that fresh coin in the same transaction, sending the whole value to `ownPublicKey()` — the caller. The recipient is wrapped with `left<ZswapCoinPublicKey, ContractAddress>(...)` because it is a user key, not a contract. Because you send the full value, there is no change coin to manage.
- The last three lines record the color, flip `initialized`, and bump the counter.

The mint amount is a `Uint<64>` (the protocol caps a single mint at that width); coin values are `Uint<128>`, which is why `coin.value` is the wider type. Unlike the unshielded tutorial's permissionless mint, this one is gated: the opening `assert` restricts minting to the wallet that deployed the contract.
...
All of the shielded TypeScript goes in `src/shielded/`. Two of the support files differ from the unshielded tutorial; the other two — `config.ts` and `wallet.ts` — are reused from `src/` unchanged, which is why the imports below reach up a level with `../`.
...
The deploy script follows the same shape as before — build the wallet, wait for funds and DUST, then deploy — and differs only in what it deploys and calls. The wallet, funding, and DUST steps are the same as the unshielded tutorial, but they are included in full here so this tutorial runs on its own.
...
The imports change to bring in the shielded contract, the private-state factory, and `toHex`, and the two shared modules are reached with `../`:
...
After syncing, the wallet should report 1,000 of the token's color in its shielded balance — proof that the contract minted shielded value and delivered it to you privately.
...
## Going further with committed transfers and burns
...
1. **Transferring a coin the contract already holds.** Once a minted coin is committed on-chain, it becomes a `QualifiedShieldedCoinInfo` (it gains a Merkle tree index, `mtIndex`), and you spend it with `sendShielded`:
...
The hard part on a live network is obtaining the `mtIndex` for the committed coin and supplying it to the circuit, typically from the indexer; that is the piece to work out and test carefully. Remember too that delivering shielded value to a user other than the caller will not notify them today.
