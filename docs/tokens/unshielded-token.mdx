...
The two `assert` lines are guards. They run as part of the circuit, and if either condition is false the assertion fails and rejects the whole call, committing nothing on-chain. The first fails the call if you haven't minted the token yet; the second fails if the contract doesn't hold enough, checked with `unshieldedBalanceGte`.
