...
sidebar_label: "Build the bulletin board contract"
title: "Build the bulletin board contract"
...
The bulletin board contract allows users to post and remove messages while maintaining privacy. Only the original poster can remove their message, and this is enforced through Zero Knowledge (ZK) proofs without revealing the poster's identity on-chain.
...
The `CompactStandardLibrary` provides access to built-in types and functions in Compact, such as `Maybe` for optional values, `Counter` for tracking sequences, `persistentHash` for cryptographic commitments, `some` and `none` constructors for Maybe values, and `pad` for creating fixed-length byte arrays.

:::note
To learn more about available types, see the [Compact standard library](../../../compact/standard-library/) reference.
:::
...
- Sets the initial `state` to `VACANT` since no messages have been posted
- Initializes `message` to `none` using explicit type annotation
- Increments `sequence` to 1 to prepare for the first post
- Leaves `owner` uninitialized, as it defaults to zero bytes

The sequence counter starts at 1 rather than 0. This makes the first post use sequence number 1, which is more intuitive for users.

:::note Constructor initialization
Fields not explicitly initialized in the constructor receive default values for their type. The `owner` field defaults to 32 zero bytes, and the `sequence` counter defaults to zero before being incremented to 1.
:::

### Declare the witness function

Before defining the circuits, declare a witness function to access private data:
...
- `witness` marks this as a function that accesses private state
- `localSecretKey` returns the user's secret key as a 32-byte value
- The return value never appears on-chain or in public ledger state

Witnesses are crucial for privacy. They allow circuits to use private data for computation and proof generation without revealing that data to validators or other network participants.
...
- **Generate commitment**: Calls `publicKey()` helper circuit to create a cryptographic commitment from the secret key and current sequence number
- **Disclose commitment**: Wraps the commitment in `disclose()` to tell the compiler this value is safe to reveal on-chain
...
The second `assert` is where privacy meets access control. The user proves they can regenerate the stored commitment without revealing their secret key. The Zero Knowledge (ZK) proof validates this assertion without exposing the private data.
...
- Takes a secret key (`sk`) and sequence number as inputs
- Returns a 32-byte cryptographic hash
- Uses `persistentHash` from the standard library for secure hashing
...
- **One-way**: You cannot reverse the hash to discover the secret key
...
- `keys/`: Cryptographic keys used for generating and verifying Zero Knowledge (ZK) proofs for each circuit
...
- Returns `CircuitResults` containing the Zero Knowledge (ZK) proof and circuit outputs
