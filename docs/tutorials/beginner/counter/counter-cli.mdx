...
This tutorial shows you how to build a command-line interface that interacts with the counter smart contract created in the [build the counter contract](./smart-contract) tutorial. 
...
This section walks you through the process of setting up the root package.
...
This section walks you through the process of setting up the CLI DApp.
...
**The workaround**: This function manually signs each intent by:
1. Deserializing the intent with the correct proof marker (`'proof'` for proven transactions, `'pre-proof'` for unproven transactions)
2. Generating signatures for the intent's signature data
3. Adding signatures to both fallible and guaranteed unshielded offers within the intent
4. Updating the transaction's intent map with the correctly signed intent
...
2. MidnightProvider interface `submitTx()`: Submits a finalized transaction to the Midnight network and returns a promise that resolves when the transaction is confirmed.
...
The monitor updates every 5 seconds and runs until the user presses Enter, providing visibility into DUST generation progress.
...
Below the menu constants, create the wallet setup flow:
...
The wallet setup flow provides two main functions:

**`buildWallet`**: Displays an interactive menu with three options:
...
Below the wallet setup flow, create the helper functions for displaying the DUST balance, joining a contract, and starting the DUST monitoring:
...
1. **Outer try-finally block**: Ensures the readline interface closes and listeners are removed, even if errors occur. This prevents memory leaks and ensures clean process termination.

2. **Inner try-finally block**: Guarantees the wallet stops properly, closing all network connections and persisting state. The wallet cleanup happens before readline cleanup to ensure all transactions complete.

3. **Error handling**: Catches and logs errors during provider configuration or main loop execution, allowing graceful shutdown instead of abrupt crashes.
...
This section walks you through the process of running the complete Counter DApp.
...
The CLI displays the welcome banner and wallet setup menu.
...
The application displays a banner and presents the wallet setup menu:
