...
```json title="counter-cli/package.json"
{
...
```json title="counter-cli/tsconfig.json"
{
  "compilerOptions": {
    "target": "ES2022",
    "module": "ES2022",
 ...
 *
 * On Preprod/Standalone, NIGHT tokens generate DUST over time, but only after
 ...
 *   5. If balance is zero, then wait for incoming funds (for example, from faucet).
 */
...
${DIV}
  ...
  ${!(config instanceof StandaloneConfig)
            ? `
...
5. **Register for DUST generation**: This step registers the wallet's NIGHT UTXOs with the DUST generation service. Preprod and Standalone networks require this step for transaction fees.
    ...
    switch (choice.trim()) {
      case '1':
        try {
          ...
          console.log(`\n  ✗ Deploy failed: ${msg}`);
          // Log the full cause chain to help debug WASM and ledger errors
...
export const run = async (config: Config, _logger: Logger, dockerEnv?: DockerComposeEnvironment): Promise<void> => {
  ...
  try {
    // Step 1: Start Docker environment if provided (for example, local proof server)
...
Create `counter-cli/src/standalone.ts`:

```typescript title="counter-cli/src/standalone.ts"
import { createLogger } from './logger-utils.js';
import path from 'node:path';
import { run } from './cli.js';
...
Create `counter-cli/standalone.yml`
```yaml title="counter-cli/standalone.yml"
services:
  indexer:
...
Create `counter-cli/standalone.env.example`

```env title="counter-cli/standalone.env.example"
# Environment variables for the standalone indexer container.
# These are dev defaults for local use only.
