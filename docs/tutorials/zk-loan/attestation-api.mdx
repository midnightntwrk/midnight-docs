...
:::note
If you are testing with [Midnight Local Dev](/guides/local-to-testnet#local-network) instead of Preprod, then skip this step. The local dev environment already includes a proof server on port `6300`.
:::

The [proof server](/guides/development-environment#run-the-proof-server) generates ZK proofs for every transaction that interacts with the smart contract. For Preprod, you run it locally via Docker while the blockchain node and indexer are remote (hosted by Midnight Network).
