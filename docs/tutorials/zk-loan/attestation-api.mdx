...
**Step 2: Request attestation.** The CLI sends the user's credit data and the derived public-key hash to the attestation API. The CLI computes the public key hash from the user's wallet key and secret PIN using the `publicKey` pure circuit from [Part 1](./smart-contract).
...
**Step 6: Record the outcome.** Only the loan status (Approved, Proposed, or Rejected) and the authorized amount are written to the ledger via [`disclose()`](/compact/reference/explicit-disclosure). The credit score, income, tenure, PIN, and attestation signature remain private.
...
:::note
The proof server image version must match your SDK version. This tutorial uses `4.0.0`. Check the [release notes](/relnotes/compact-js) for the latest compatible version.
:::
...
The attestation API can sign credit data. The proof server can generate proofs. The smart contract from [Part 1](./smart-contract) is compiled and deployed. The missing piece is the interface that ties them all together.

## Coming up in [Part 3](./cli)
