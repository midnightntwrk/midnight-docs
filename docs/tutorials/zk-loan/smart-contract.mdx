...
[Zero-knowledge proofs](/concepts/zero-knowledge-proofs) flip this model. Instead of showing your data, you prove a statement about it: "My credit score is above 700, and my income exceeds $2,000/month," and the verifier learns nothing else. Not the exact score. Not the exact income.
...
1.  **Smart contract**: Written in [Compact](/compact) (Midnight's zero-knowledge language), this defines the loan logic, eligibility tiers, and on-chain state.
...
Install the Midnight Compact compiler. Follow the instructions in the [installation guide](/getting-started/installation) to install `compactc` on your system.
...
1.  Gets the user's credit profile, Schnorr signature, and provider ID from the witness

2.  Verifies the provider is registered on-chain

3.  Verifies the Schnorr signature — this proves the data came from a trusted provider and was not fabricated by the user

4.  Evaluates the credit profile against three tiers:
...
1.  Process exactly **5 loans per transaction** (fixed at compile time with `for (const i of 0..5)`)

2.  Track progress in `onGoingPinMigration` — it stores how far the migration has gotten

3.  The user calls `changePin` **repeatedly** until all loans are migrated

4.  Once done, the migration state is cleaned up
...
1.  The (possibly updated) private state

2.  The values the circuit requested
...
## Coming up in [Part 2](./attestation-api)

In the next part, you build the off-chain infrastructure that makes the smart contract functional:
