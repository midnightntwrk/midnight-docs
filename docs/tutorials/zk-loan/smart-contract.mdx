...
This tutorial builds a zero-knowledge loan application from scratch on [Midnight Network](/concepts/network-architecture/onchain-logic). The app privately evaluates a user's credit data (credit score, income, and employment tenure) using [zero-knowledge proofs](/concepts/zero-knowledge-proofs) and records only the loan outcome on-chain. The sensitive financial data never leaves your machine.

Traditional lending requires sharing sensitive financial data with lenders, brokers, and underwriters. That data gets stored, shared, and inevitably leaked. In 2024 alone, financial data breaches exposed hundreds of millions of records. Zero-knowledge proofs flip this model. Instead of showing your data, you prove a statement about it: "My credit score is above 700, and my income exceeds $2,000/month," and the verifier learns nothing else. Midnight Network makes this practical — it is a blockchain purpose-built for [data protection](/concepts/how-midnight-works/keeping-data-private), where smart contracts can process private inputs and commit only the results on-chain.

The tutorial is split into three parts:

1.  **Smart contract**: Written in [Compact](/compact) (Midnight's zero-knowledge language). It defines the loan logic, eligibility tiers, and on-chain state.
...
Verify it is available (this tutorial targets Compact toolchain `0.30.0`):
...
Also, Docker is required to run the Midnight proof server, which generates the zero-knowledge proofs for your transactions locally. Install it from [docker.com/get-docker](https://www.docker.com/get-docker) if you have not already, and make sure the Docker daemon is running.
...
This first block declares the language version, imports, data types, ledger state, and the constructor that derives the initial admin public key from a witness-provided secret:
...
// Admin authorization uses the witness-derived keypair pattern. The admin
// holds a 32-byte secret in private state; the ledger stores only its
// public hash. Every admin circuit asserts the caller's secret hashes to
// the stored value — `ownPublicKey()` is never used for authorization.
export struct AdminSecretKey {
    bytes: Bytes<32>;
}
...
// Domain-separated hash binds this admin secret to this specific contract,
// so the same secret cannot be reused across other deployments.
...
- `contractAdmin` stores the *derived public key* of the admin: `persistentHash("zkloan:admin:pk:v1" || adminSecret)`. The deploying admin holds the 32-byte secret in private state; the ledger only ever sees the hash. Each admin circuit forces the caller, inside the ZK proof, to demonstrate knowledge of the preimage of this stored value.
...
- `deriveAdminPublicKey(sk)` is a pure circuit that hashes a secret with a contract-specific domain separator. The constructor calls it with the witness `getAdminSecret()` to compute the initial admin's public key.

Finally, the `witness` declarations tell the compiler: "At proving time, TypeScript code will provide an `Applicant` + `SchnorrSignature` + provider ID, and separately a 32-byte admin secret." The TypeScript implementation is covered in the next section.
...
```compact
assert(contractAdmin == deriveAdminPublicKey(getAdminSecret()), "Only admin can ...");
```

Inside the ZK proof, this enforces that the caller knows the 32-byte preimage of the public value stored in `contractAdmin`. The ledger value alone is useless to an attacker: they could copy it into their proof input, but they cannot supply a witness that hashes to it without knowing the secret.

:::warning Why not `ownPublicKey()`?

An older pattern was `assert(ownPublicKey() == admin, "...")`. This is **not** authorization. `ownPublicKey()` returns a value the prover claims; it is not a proof of key ownership. Because `admin` was a public ledger value, any reader of the chain could supply that value as their own `ownPublicKey()` and the assertion would pass. The witness-derived keypair pattern fixes this by forcing the caller to know a secret whose hash is on the ledger.

`ownPublicKey()` remains a valid primitive when used as an *identifier* (e.g. blacklist membership lookup, deriving a PIN-bound per-user key) — never as the right-hand side of an authorization assert.

:::

The five admin circuits are:

*   `blacklistUser` / `removeBlacklistUser`: Add or remove a wallet from the blocklist.

*   `registerProvider` / `removeProvider`: Add or remove an attestation provider's public key.

*   `rotateAdmin`: Hand over the admin role to a new derived public key. The new admin generates their own secret locally and shares only the resulting public key — no private key crosses the wire.
...
export circuit removeBlacklistUser(account: ZswapCoinPublicKey): [] {
    assert(contractAdmin == deriveAdminPublicKey(getAdminSecret()), "Only admin can remove from blacklist");
...
export circuit rotateAdmin(newAdmin: AdminPublicKey): [] {
    assert(contractAdmin == deriveAdminPublicKey(getAdminSecret()), "Only admin can rotate admin role");
...
*   `getAdminSecret`: Returns the 32-byte admin secret from private state. The circuit hashes it with the contract-specific domain separator and compares the result to the `contractAdmin` ledger value. Only the deploying admin holds a secret whose hash matches what is stored on-chain; for everyone else, the comparison fails and the transaction reverts. The runtime validation ensures the witness never returns a malformed value.
