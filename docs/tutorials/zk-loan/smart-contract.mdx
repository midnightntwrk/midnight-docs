...
The workspace is pinned to `0.30.0`. Run `compact update` to install all available compilers; the build script (below) invokes the pinned version explicitly with `compact compile +0.30.0`, so even with newer compilers installed the project will compile against `0.30.0`.

:::note Toolchain pin

The toolchain pin is required while `@midnight-ntwrk/compact-js@2.5.0` hard-pins `@midnight-ntwrk/compact-runtime` to exactly `0.15.0`. The latest compactc (0.31+) emits code expecting runtime `0.16`, which the workspace cannot reach until compact-js publishes a `0.16`-compatible release. Until then, run `compact update` to install all available compilers, but invoke the workspace build with `compact compile +0.30.0`.
...
*   `blacklistUser` / `removeBlacklistUser`: Add or remove a derived `UserPublicKey` from the blocklist. The admin obtains the target's `UserPublicKey` from the on-chain `loans` map keys (where it appears whenever the target has interacted) or from the target out-of-band. An admin cannot blacklist by wallet address — `ownPublicKey()` is not trusted by the contract.
...
| `contractAdmin`, blocklist | Public (ledger) | Smart contract governance |
...
cat > contract/package.json << 'EOF'
{
  ...
  "scripts": {
    "compact": "compact compile +0.30.0 src/zkloan-credit-scorer.compact src/managed/zkloan-credit-scorer",
    "test": "vitest run",
