...
Only the original poster can remove their message, and this is enforced through zero-knowledge (ZK) proofs without revealing the poster's identity on-chain.
...
- **Proof server running**: For instructions, refer to the [run the proof server](../../../getting-started/installation#set-up-the-proof-server) guide
- **Node.js version 24+**: Verify with `node --version`
...
- Locks your contract to a minimum Compact version
...
- `moduleResolution`: Set to `Bundler` to resolve subpath imports from the Midnight.js packages
- `strict`: Enables strict type checking for better code quality

Create a `tsconfig.build.json` file in the same directory to exclude test files from the production build:
...
- `compact`: Runs the Compact compiler to generate circuits and TypeScript API
- `typecheck`: Verifies TypeScript types without emitting JavaScript output
- `build`: Compiles TypeScript using `tsconfig.build.json` and copies the generated `managed/` directory and contract source into `dist/`

### Build the contract

The `compact` and `build` scripts run independently so that you can re-run the TypeScript build without recompiling the Compact contract:
...
1. Install dependencies (TypeScript compiler)
2. Compile the Compact contract to circuits and the generated TypeScript API under `src/managed/bboard/`
3. Compile TypeScript to JavaScript using `tsconfig.build.json`, which excludes the `src/test` directory
4. Copy the `managed/` directory and the source `bboard.compact` file into `dist/`

If successful, you will have:

- `src/managed/bboard/`: Generated contract code from the Compact compiler
- `dist/`: Compiled JavaScript and type definitions
- `dist/managed/bboard/`: Generated contract code copied for packaging
