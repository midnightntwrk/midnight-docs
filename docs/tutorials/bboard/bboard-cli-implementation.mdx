...
- Filters unregistered UTXOs
- Creates a DUST generation transaction
- Signs it with the unshielded keystore
- Submits it to the network
- Waits for the DUST balance to update
...
The function provides three options: generate a new random seed, restore from an existing seed, or exit. Users should save the seed securely if they want to access the same wallet in future sessions.
...
:::info
The `undeployed` launcher connects to a local Midnight network running on your machine. For more information, see the [Midnight local network](../../../guides/midnight-local-network/) documentation.
:::
</TabItem>
</Tabs>
