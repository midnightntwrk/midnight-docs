...
The `isFacadeStateSynced` helper centralizes the three-way progress check (shielded, unshielded, dust) so it can be reused by `waitForUnshieldedFunds`. The DUST balance is read via `state.dust.balance(...)` — the older `walletBalance(...)` API has been removed in `@midnight-ntwrk/wallet-sdk` 1.0.
...
`waitForUnshieldedFunds` waits until the wallet is fully synced **and** the unshielded balance for the requested token type is greater than zero — this avoids races where the wallet finishes its initial sync before the faucet credit has been observed.
...
The loop first calls `deployOrJoin()` to initialize the contract connection. It subscribes to the reactive state observable to automatically receive updates whenever transactions modify the contract. Each menu action is wrapped in its own `try`/`catch` so that a failed circuit invocation (for example, posting to an occupied board) logs the error and returns to the main menu instead of exiting. The outer `finally` block ensures the subscription is cleaned up when the loop exits.
