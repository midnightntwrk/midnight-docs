...
Compact provides a significant amount of flexibility for a DApp developer to precisely define a mix of public and private data. This tutorial will focus primarily on the Compact code and the remainaing setup code comes with the repository.
...
The implementation for `getDappPublicKey()` will come later, for now, just understand that the organizer provides a secret that is hashed to provide a "public key" that is specific to this DApp. Compact circuit inputs are private, so the `_secret` is passed from private state directly to the circuit inputs and never exposed publicly. The `pubKey` is safe to store on the ledger publicly, because it does not tie to the organizers address or any other identifying information.
...
To inspect the full private-party repository see [example-private-party](https://github.com/midnightntwrk/example-private-party). Drop a message in Discord dev-chat that you've completed this tutorial and you are now a party planner or if you *mustache* any questions.
