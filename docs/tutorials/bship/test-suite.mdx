...
First, the project is set up for testing. Copy and paste the following configuration files.
...
Finally, add the docker configuration for deploying to local devnet:
```bash
touch compose.yml
```
...
Now that the contract is deployed it should have an initial state on the blockchain. Use the `queryLedger()` function to check that the state is as expected:
```typescript
