...
The `PLAYER_1_SHOOT` circuit will be available first and accesses to others will be blocked. After a successful state transition `PLAYER_2_CHECK` will be "unlocked" and access to `PLAYER_1_SHOOT` will be unavailable.
...
Now that they have committed to these values publicly, ask the the player to set their board locally:
```compact
...
Before going any further in the game flow, the necessary hashing functions can be implemented:
```compact
...
Finally, on-chain data and state can be updated:
```compact
...
`player2` has the same needs as `player1` in this context, the same circuit can be implemented for them with differences in identifiers specific to the appropriate player:
```compact
...
`player2` has the same needs as `player1` here, so the same circuit can be implemented  with different identifier names specific to the appropriate player:
```compact
...
Before moving on to testing the contract, the necessary witness functionality need to be implemented. From inside the `/contract` directory:
```bash
cd contract
touch witnesses.ts
```
...
Next implement a "setter" of private state data:
```typescript
    localSetBoard: ({
        privateState
...
That is everything needed for off-chain witnesses, the compiled contract can be now be exported.
