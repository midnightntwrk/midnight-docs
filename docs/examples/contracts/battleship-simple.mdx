...
sidebar_position: 6
title: Battleship Simple Version
...
For the full DApp example with complete frontend test suite, see [example-battleship](https://github.com/midnightntwrk/example-battleship).

```compact
// Battleship: Two-player ZK game. Player 1 deploys (sets ships).
// Player 2 joins via acceptGame(). Turn order:
//   PLAYER_1_SHOOT → PLAYER_2_CHECK → PLAYER_2_SHOOT → PLAYER_1_CHECK → repeat
// Ship locations are committed via persistentHash; ZK proves honest hit/miss reports.
// Win condition: 2 hits on the opponent's ships.
...
constructor(_x1: Uint<8>, _x2: Uint<8>) {
    ...
    // best practice example -- don't disclose(localSetBoard(_x1, _x2)), 
...
export circuit checkBoard1(): [] {
    ...
    // hash for comparison with on-chain hash
