...
- **Witness-derived admin and identity**: caller identity (both admin and per-user) is derived from a 32-byte witness secret. `ownPublicKey()` is not used by the contract. Only the holder of the admin's secret can register providers, blocklist users, or rotate the admin role; only the holder of a user's secret can request, accept, or decline that user's loans.
...
- **Compact toolchain (latest)** — install with the `compact` devtool, then run `compact update`. Verify with `compact compile --version` (this tutorial was last validated against `0.31.0`).
