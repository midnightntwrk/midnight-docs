...
This example targets the following versions from the [compatibility matrix](../../relnotes/support-matrix).
...
This section covers the process of building and running the leaderboard DApp locally. The full step-by-step build instructions are in the [Leaderboard tutorial](../../tutorials/leaderboard/overview). Follow the tutorial to create the project from scratch, then return here for the run instructions.

If you have already completed the tutorial and have the project directory ready, continue from the next section.

## Start the proof server

The proof server generates ZK proofs for transactions locally to protect private data. It must be running before you can compile or interact with contracts.
...
Open a new terminal window and navigate to your project directory. Install all workspace dependencies from the repo root:

```bash
npm install
```

Navigate to the `contract` directory and compile the contract:
...
The `dev` script copies the compiled circuit keys and ZKIR files into `public/` before starting Vite:

```bash
mkdir -p ./public/keys ./public/zkir &&
  cp -r ../contract/managed/leaderboard/keys/* ./public/keys/ &&
  cp -r ../contract/managed/leaderboard/zkir/* ./public/zkir/ &&
  vite
```
