...
- Using Zero Knowledge (ZK) proofs to verify identity without revealing it
...
If you already have a wallet seed, select option `2` to build wallet from a seed.
...
If you want to interact with a contract that someone else deployed, select option `2` to join an existing bulletin board contract.
...
You can now interact with the bulletin board just as if you had deployed it yourself.
...
2. Creates a Zero Knowledge (ZK) proof proving you followed the contract rules
...
3. Submits a Zero Knowledge (ZK) proof of this verification
4. Removes the message and resets the board to vacant

:::caution Only poster can remove
If you try to take down someone else's message, the transaction fails. The contract enforces this through Zero Knowledge (ZK) proof verification that happens before submission.
:::
