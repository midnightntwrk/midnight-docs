...
- Added `connect` method and `submitTx` `TransactionIdentifier`.
- Implemented transaction cost calculation.
- Enabled state updates from the network.


### Upgrade Instructions

- Ensure compatibility with the new `connect` method and `TransactionIdentifier` in `submitTx`.
- Updated ledger serialization encoding to `base64`.
