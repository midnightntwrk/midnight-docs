...
Compact compiler version 0.27.0 is being release today.
This version compiles Compact language version 0.19, which is an updated version of the language.

:::important

This is the first version of the compiler which targets the new "Pre-Production Testnet" and "Preview Testnet" environments of the Midnight Network.
...
Similarly, contracts compiled with version 0.26 or earlier will not work with the Pre-Production or Preview environments.

:::

This release is part of an important milestone for the Midnight Network: the launch of the new Pre-Production Testnet and Preview Testnet environments.
Read these release notes carefully to note the breaking changes.

### Summary of changes

The Pre-Production and Preview environment are using Midnight ledger version 6 (compared to testnet's ledger version 4).
...
This release has numerous bug fixes and the usual performance improvements.
...
Note that this works for the standard library too, because `CompactStandardLibrary` is a module.

This is a non-breaking change and the original form of import is still supported.
...
These were changed these because they were inconsistent with ranges elsewhere in the language, e.g., in `for` loops.
...
The `compact fixup` tool will not update these automatically, but you can pass a flag `--update-Uint-ranges` to it and it will attempt to fix them;
...
`Uint` ranges for bounded unsigned integers, of the form `Uint<0..n>` (**not** sized unsigned integers of the form `Uint<n>`)
are changed to be limited to 248 bits.
They were previously limited to 254 bits (which is not quite a full 32 bytes).

The reason for this change is that unsigned integers that actually used more than 248 bits were not correctly represented in the ZK circuit and on chain.
To avoid erroneous conversions, the range has been limited.

This is a breaking change for the bounded `Uint` type.
...
It is better in that case for the compiler to fail to compiler, instead of learning of the problem later when a contract is deployed.
...
One bug fix is a breaking change: previously not all unreachable statements were reported as such by the compiler.
