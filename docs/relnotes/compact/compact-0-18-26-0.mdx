---
title: Compact compiler 0.26.0 (Compact language 0.18.0)
displayed_sidebar: sidebar
---

## Compact compiler 0.26.0 (Compact language 0.18.0) release notes

Today we are releasing version 0.26.0 of the Compact compiler.
This release updates the Compact language from version 0.17.0 to 0.18.0.
...
The difference in Compact between a bytes value and a vector is that the bytes value has a "packed" representation in proofs and on chain,
...
This new feature is a **non-breaking** change (because `Bytes` was already a keyword).
...
The looping forms in Compact now work with bytes values.
...
The reason for tthe choice to return a vector for `map` over a bytes value is to avoid the potentially expensive packing of a vector into a bytes value when it's not needed.
...
- Hexidecimal literals are preceded with either `0x` or `0X`,
  and the digits are the numeric digits in the range `0..9` and the alphabetic digits in the ranges `a..f` and `A..F`.
- Octal literals are preceded with either `0o` or `0O`, and the digits are the numeric digits in the range `0..7`.
- Binary literals are preceded with either `0b` or `0B`, and the digits are the numeric digits in the range `0..1`.
...
and you cnanot spread tuples inside bytes value creation expression (again without using a cast of some kind).
...
And the resulting type will be `Vector<N+2, S>` where `S` is the least upper bound with respect to subtying of `T`, `Uint<0..0>`, and `Uint<0..255>`.
...
We experemented with various syntax ideas for this feature, and settled on one that we hope is easy to use but still allows us to give correct static types to such expressions.
...
- `SIZE` is the length of the slice.
...
You will be tempted to want to also perform arithmetic on generic size parameters in **types**, but this does not (yet) work in Compact.
...
Previously, Compact supported a limited set of type casts.
...
When a cast is possible, we have considred providing the ability to cast in both directions.
...
- From `[T0, ..., Tn]` to `[S0, ..., Sn]`, tuple types with the same length and where each `Si` is a supertype of the corresponding `Ti`.
...
- From `Vector<n, T>` to `[S0, ..., Sn]`, a vector type to a tuple type with the same length, where each `Si` is a supertype of `T`.
  It cannot fail and there is no runtime cost.
  Note that there is generally no corresponding cast in the opposite direction.
  
- From `[T0, ..., Tn]` to `Vector<n, S>`, a tuple type to a vector type with the same length, where `S` is a supertype of each `Ti`.
  It cannot fail and there is no runtime cost.
  Note that there is generally no corresponding cast in the opposite direction.
...
- From `Bytes<n>` to `Vector<n, T>`, a bytes value type to a vector type of the same length, where `T` is a supertype of `Uint<8>`.
  ...
  This cost is the same as the cost of a cast to `Vector<n, Uint<8>>` (the cast from `Uint<8>` to a supertype is "free").
  
- From `Bytes<n>` to `[T0, ..., Tn]`, a bytes value to a tuple type of the same length, where each `Ti` is a (possibly different) supertype of `Uint<8>`.
  Since vector types are equivalent to a corresponding tuple type, the cast above is a special case of this one.
...
- From `enum` types to `Uint` types and vice versa
  ...
  The cast to an `enum` type is checked in both JavaScript and in the proof, and it involves a representation change (from `bigint` to `number`) in JavaScript.
  The cast to `Uint` types does not need a runtime check, but it involves a JavaScript representation change.
...
and we changed them to mention the Compact types involved, not the JavaScript representation types.
...
It was possible in some rare cases to crash our circuit optimzation with an internal compiler error.
