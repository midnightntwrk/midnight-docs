...
Ledger 7.0.0 is a major release that switches to the Midnight Structured Reference String (SRS) with midnight-zk 1.0, and adds the new `addCalls` endpoint for transcript partitioning with zswap components. 
...
Allows creating offers without knowing the target segment or re-stating values in deltas. 
...
Exposed Dust parameters (`sync_time`, etc.) in `DustLocalState` for better state introspection.
