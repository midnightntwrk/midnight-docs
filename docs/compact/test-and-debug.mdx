...
Compact smart contracts execute across multiple contexts (on-chain ledger, zero-knowledge circuits, and local witnesses), requiring different testing strategies for each layer. 
Proper testing ensures your contract behaves correctly, maintains privacy guarantees, and handles edge cases appropriately.

This guide covers the following testing strategies:
...
The following example tests the basic increment operation for the [Counter contract](../tutorials/counter/smart-contract). This demonstrates unit testing at the circuit level without requiring a full blockchain deployment:
...
Test that your contract enforces valid state machine transitions and rejects invalid ones. The [bulletin board](../tutorials/bboard/smart-contract) contract demonstrates two-state validation:
