...
Testing Midnight smart contracts involves verifying that your Compact circuits behave correctly, TypeScript witnesses integrate properly, and that the full transaction lifecycle — proof generation, submission, and ledger state updates — work as expected.

Midnight provides `@midnight-ntwrk/testkit-js`, a dedicated testing library that manages environment setup so you can focus on writing contract features.

## What you should test

Compact contracts have three layers, each benefitting from testing:
...
The test file is where you implement different interactions with many instances of the deployed contract. Add the following code to your test file: 
