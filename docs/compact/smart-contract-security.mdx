...
It is best practice for DApp developers to isolate functions requiring access to private state to witnesses, but this is not enforced programatically. Due to the strong adherence to the "private by default" model, private state data can also be supplied directly to circuit inputs(internal or exported) and still remain private.
