...
Before writing or deploying smart contracts, you’ll begin by becoming a user of the Midnight network. This involves setting up the Midnight Lace wallet, installing a local proof server, and acquiring tDUST (test tokens) to interact with the blockchain.
...
* The Midnight Lace Wallet (alpha) Chrome extension, specific to the testnet version.
...
### Install the Midnight Lace Wallet

Download the latest testnet-compatible version of the Midnight Lace Wallet from here. After unzipping the .zip file, load it into Chrome manually:

1. Open Chrome → Extensions → Enable Developer Mode.
2. Click Load Unpacked, then select the unzipped wallet folder.
...
* A Midnight Lace Wallet with tDUST
...
<QuizWithRetry
  questions={[
    ...
    {
      question: 'Which browser is fully supported for the Midnight Lace wallet extension?',
...
export ledger public_data: Bytes<32>;

export circuit record(): [] {
  public_data = disclose(get_secret());
}
```
...
```
persistent_commit(value, random)   // Creates a public commitment
transient_commit(value)            // Creates a temporary commitment
```
...
Midnight provides a development environment for building privacy-preserving DApps. First, you set up as a user by installing Docker, Chrome, and the Midnight Lace Wallet, then acquiring test tokens (tDUST) from the faucet. You also run a local proof server to handle zero-knowledge proofs. Next, you prepare for development by installing Node.js, the Compact compiler, and Midnight TypeScript libraries. Optionally, you can install the Compact VSCode extension for better coding support. With everything set up, you’re ready to write, compile, and deploy your own Midnight smart contracts.
